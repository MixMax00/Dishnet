...
@php($package_list = \App\Models\AdminPackage::orderBy('id','DESC')->get())
                        ...
                        <tbody>
                            ...
                            @foreach($package_list as $key => $list)
                                <tr>
                                    ...
                                    <td>
                                        <a href="{{ route('superadmin.package.status', ["id"=>$list->id]) }}" class="btn btn-{{ $list->status == 1 ? "warning" : "primary" }} btn-sm text-dark m-1">{{ $list->status == 1 ? "Inactive" : "Active" }}</a>
                                        <a href="{{ route('superadmin.package.edit', ["id"=>$list->id]) }}" class="btn btn-info btn-sm text-white m-1">Edit</a>
